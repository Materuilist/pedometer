...
function Graph({ activities }) {
  function prepareFooterData() {
    ...
    const min = convertDistance(activities.reduce(
      (minDistance, { distance }) =>
        +distance < minDistance ? +distance : minDistance,
      50001
    ));
    const max = convertDistance(activities.reduce(
    ...
    const sumLastWeek = convertDistance(activities
      .filter(({ date }) => currentDate - Date.parse(date) <= MILISECONDS_IN_WEEK)
